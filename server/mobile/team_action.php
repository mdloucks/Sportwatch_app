...
// -- Check their SID -- //
if((getVar("SID") == false) || (!$db->validateSID(getVar("SID")))) {
    sendResponse(array("status" => -5, "substatus" => 2, "info" => "invalid SID"));
}
// Will check the SID against the user id below
$userIdFromSID = $db->selectSingle("SELECT id_user FROM user_session_data WHERE SID = ?", "s", getVar("SID"));
$userIdFromSID = $userIdFromSID[0];
...
if($intent === false) {
    sendResponse(array("status" => -5, "substatus" => 4, "info" => "no intent"));
}
if(!getVar("account_email")) {
    sendResponse(array("status" => -5, "substatus" => 4, "info" => "no user email"));
}
...
if((count($accountId) > 0) && ($db->emailExists(getVar("account_email")))) {
    ...
    sendResponse(array("status" => -5, "substatus" => 8, "info" => "provided email: " . getVar("account_email")));
}
// Check the SID against user ID
if($accountId != $userIdFromSID) {
    sendResponse(array("status" => -5, "substatus" => 2, "info" => "invalid SID"));
}
...
if((strlen($teamName) <= 5) || (strlen($teamName) > 75)) {
    $teamName = "";
}
...
if($targetTeamId !== false) {
    ...
    if($targetTeamId < 0) {
        ...
        sendResponse(array("status" => -5, "substatus" => 6, "info" => 
...
// -- Send team roster --//
    ...
    if((count($enrolledName) > 0) || (count($coachArray) > 0)) {
...
// -- Join team -- //
    ...
    if($teamData["is_locked"] == false) {
        ...
        sendResponse(array("status" => -5, "substatus" => 3, "info" => "team is locked"));
    }
...
// -- Leave team -- //
    ...
    if(!$db->isInTeam($accountId, $targetTeamId)) {
        sendResponse(array("status" => -5, "substatus" => 3, "info" => "user not in team"));
    }
    ...
    if($accountId == $teamData["id_coach_primary"]) {
        if($teamData["id_coach_secondary"] != NULL) {
            ...
            sendResponse(array("status" => -5, "substatus" => 3, "info" => "only coach cannot leave"));
        }
...
// -- Kick from team -- //
    ...
    if(($accountId != $teamData["id_coach_primary"]) && ($accountId != $teamData["id_coach_secondary"])) {
        sendResponse(array("status" => -5, "substatus" => 3, "info" => "coach-only action"));
    }
    if($kickedId < 0) {
        sendResponse(array("status" => -5, "substatus" => 4, "info" => 
    ...
    if($kickedId == $teamData["id_coach_primary"]) {
        sendResponse(array("status" => -5, "substatus" => 3, "info" => "cannot kick primary coach"));
    }
...
// -- Appoint primary coach -- //
    ...
    if($promotedId == $teamData["id_coach_primary"]) {
        sendResponse(array("status" => 5, "substatus" => 3, "info" => "already primary coach"));
    }
    ...
    if($accountId == $teamData["id_coach_primary"]) {
        ...
        sendResponse(array("status" => -5, "substatus" => 3, "info" => "primary coach-only action"));
    }
...
// -- Appoint secondary coach -- //
    ...
    if(($accountId == $teamData["id_coach_primary"]) || ($accountId == $teamData["id_coach_secondary"])) {
        ...
        if(($promotedId == $teamData["id_coach_primary"]) || ($promotedId == $teamData["id_coach_secondary"])) {
            sendResponse(array("status" => 5, "substatus" => 3, "info" => "already a coach"));
        }
...
// -- Demote coach -- //
    ...
    if($accountId == $teamData["id_coach_primary"]) {
        if($demotedId == $teamData["id_coach_primary"]) {
            if($teamData["id_coach_secondary"] != NULL) { // Swap primary and secondary
                ...
                sendResponse(array("status" => -5, "substatus" => 3, "info" => "team must have primary coach"));
            }
    ...
    // If the secondary coach is doing the demoting...
        ...
        if($demotedId == $teamData["id_coach_primary"]) {
            sendResponse(array("status" => -5, "substatus" => 3, "info" => "secondary coach cannot demote primary coach"));
        } else if(($demotedId == $teamData["id_coach_secondary"]) && ($teamData["id_coach_primary"] != NULL)) {
            $success = $db->modifyRecord("team", array("id_coach_secondary" => NULL), "id_team", $targetTeamId);
        } else {
            sendResponse(array("status" => -5, "substatus" => 3, "info" => "team cannot be coach-less"));
        }
...
// -- Change team name -- //
    ...
    if((strlen($newName) <= 5) || (strlen($newName) > 75)) {
        sendResponse(array("status" => -5, "substatus" => 4, "info" => "invalid or short or long name: " . $newName));
    }
    if(($accountId == $teamData["id_coach_primary"]) || ($accountId == $teamData["id_coach_secondary"])) {
...
// -- Uknown intent -- //
    ...
    sendResponse(array("status" => -5, "substatus" => 5, "info" => "unknown intent"));
}
 ...
 * @param Integer $varSource - [default = 0] 0 for POST, 1 for GET variable
 */
function getVar($varName, $varSource = 0) {
